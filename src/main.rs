...
use inkwell::targets::{TargetMachine, Target, TargetTriple, CodeModel, RelocMode, InitializationConfig, FileType};
...
fn create_target() -> TargetMachine {
    ...
    let triple = TargetTriple::create("i386-unknown-none");
...
fn main() {
    ...
    let module = context.create_module("fuckOS");
    ...
    // Set pointer to vga buffer
    let i64_type = context.i64_type();
    let vga_buffer_addr = i64_type.const_int(0xB8000, false);
    builder.build_store(ptr, vga_buffer_addr);

    // Read code into string program
    let source_filename = matches.value_of("INPUT").unwrap();
    let mut f = File::open(source_filename).map_err(|e| format!("{:?}", e)).unwrap();
    ...
    target_machine.write_to_file(&module, FileType::Object, output_file.as_ref()).map_err(|e| format!("{:?}", e)).unwrap();
}
